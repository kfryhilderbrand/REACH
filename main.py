...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Use 'all' or provide comma-separated sensor IDs to include, for example '17738,21263'.",
    )
    parser.add_argument(
        "--sensor-exclude",
        type=str,
    ...
    if args.sensor_include is None and args.sensor_exclude is None:
        ...
        include_sensor_ids = (
            None
            if args.sensor_include is not None and args.sensor_include.strip().lower() == "all"
            else parse_sensor_id_list(args.sensor_include)
        )
...
def parse_sensor_selection(selection: Optional[str]) -> tuple[Optional[set[int]], Optional[set[int]]]:
    """
    Parse a single sensor selection string.

    Supported forms:
    - "all" or blank -> use all sensors
    - "17738,21263" -> include only those sensors
    - "include: 17738,21263" -> include only those sensors
    - "exclude: 17794,21146" -> exclude those sensors
    """
    if selection is None:
        return None, None
    ...
    if lowered.startswith("exclude:"):
        return None, parse_sensor_id_list(text.split(":", 1)[1].strip())
    if lowered.startswith("exclude "):
        return None, parse_sensor_id_list(text.split(None, 1)[1].strip())
    if lowered.startswith("include:"):
        return parse_sensor_id_list(text.split(":", 1)[1].strip()), None
    if lowered.startswith("include "):
        return parse_sensor_id_list(text.split(None, 1)[1].strip()), None

    return parse_sensor_id_list(text), None


def prompt_for_sensor_filters() -> tuple[Optional[set[int]], Optional[set[int]]]:
    """
    Ask the user for sensor filtering in a single prompt.

    Supported responses:
    - all
    - comma-separated include list like 17738,21263
    - exclude: 17794,21146
    """
    prompt_text = (
        "Select sensors to process:\n"
        "- Enter 'all' to use every sensor\n"
        "- Enter sensor IDs like: 17738,21263\n"
        "- Or exclude sensors like: exclude: 17794,21146"
    )
    ...
    try:
        ...
        print(prompt_text)
        selection = input("Sensors to process [all]: ").strip() or "all"

    return parse_sensor_selection(selection)
